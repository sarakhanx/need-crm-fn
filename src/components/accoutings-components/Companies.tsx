...
const Companies = () => {
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    const { company_name, company_address, company_contact, company_vat_id } =
      company;
    if (!logo) return;
    ...
    try {
      ...
      const contactData: ContactData = address.reduce(
        (acc: ContactData, cur) => {
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit}>
            ...
            <label className="block text-gray-700 text-sm font-bold mb-2 border-2 p-3 rounded-lg w-full">
              ชื่อบริษัท
              <Input
                type="text"
                name="company_name"
            ...
            <div className="flex justify-between">
              ...
              <Button className="bg-blue-700 text-white font-bold py-2 px-4 rounded-lg w-full">
                บันทึก
              </Button>
            </div>
          </form>
